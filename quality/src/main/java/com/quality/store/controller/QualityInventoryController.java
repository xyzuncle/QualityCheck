...
import com.quality.store.service.IQualityInventoryService;
...
@Api(value = "QualityInventoryController}",description = "具体API")
@Controller
@RequestMapping("/QualityInventory")
public class QualityInventoryController extends BaseController<QualityInventory,IQualityInventoryService>{

private final Logger logger=LoggerFactory.getLogger(QualityInventoryController.class);
...
* @return
*/
@ApiOperation(value = "QualityInventory多条件查询", notes = "多条件查询")
...
public PageResult queryConditionPage(HttpServletRequest request){
        PageResult<QualityInventory> QualityInventoryListPage=null;
    try{
        //把查询条件都写好了
        Map<String, Object> searchParams=Servlets.getParametersStartingWith(request,"search-");
        //如果需要按多个字段排序，请传多个参数,为了反射方便，数据库不使用下划线了
        Sort sort = new Sort(Sort.DESC,Tools.str2StrArray("crtTime") );
        QualityInventoryListPage=(PageResult<QualityInventory>) queryContion(searchParams,sort);
        QualityInventoryListPage.setMsg("查询成功");
        return QualityInventoryListPage;
    }catch(Exception e){
        e.printStackTrace();
...
@ApiOperation(value = "增加/修改QualityInventory信息",
        notes = "保存和修改QualityInventory信息")
@RequestMapping(value = "/save.do", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
@ResponseBody
public Object saveOrUpdate(@RequestBody QualityInventory QualityInventory){
    boolean result=false;
    try{
        result=this.defaultDAO.saveOrUpdate(QualityInventory);
    }catch(Exception e){
        e.printStackTrace();
...
@ApiOperation(value = "根据Id删除QualityInventory信息")
@RequestMapping(value = "/removeById.do", method = RequestMethod.DELETE, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
@ResponseBody
public Object deleteQualityInventoryById(@ApiParam(value = "QualityInventoryID") @RequestParam(name = "entityID")  String entityID){
        boolean result=this.defaultDAO.removeById(entityID);
        return super.jsonObjectResult(result,"删除成功");
}


@ApiOperation(value = "根据ID获取QualityInventory的基本信息")
@RequestMapping(value = "/queryById.do", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
@ResponseBody
public Object queryById(@ApiParam(value = "QualityInventory唯一标识") @RequestParam(name = "id") String id){
    try{
            QualityInventory QualityInventory =this.defaultDAO.getById(id);
            return super.jsonObjectResult(QualityInventory,"查询成功");
        }catch(Exception e){
            e.printStackTrace();
            throw new BaseException("查询失败",500);
        }
...
@ResponseBody
public Object getQualityInventoryList(HttpServletRequest request){
    try{
    //把查询条件都写好了
        Map<String, Object> searchParams=Servlets.getParametersStartingWith(request,"search-");
        List<QualityInventory> list=super.queryContionNoPage(searchParams);
